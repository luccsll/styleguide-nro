import { _handleProcessError } from './errorHandler.min.js';
...
export function _setStep(id, stepsArray, infoProcess) {
    ...
    stepsArray.forEach((step, index) => {
        ...
        if (!isNaN(Number(step.responsible))) {
            var a = getDataColleague(step.responsible)

            if (!a || !a['colleagueName']) {
                _handleProcessError(`Erro ao buscar o responsável pela etapa ${step.activity}. - ERRO 002`)
                return
            }

            responsible = a['colleagueName'].length > 14 ? a['colleagueName'].substring(0, 14) + '...' : a['colleagueName']
        } else {
            responsible = step.responsible.length > 14 ? step.responsible.substring(0, 14) + '...' : step.responsible
        }
    ...
    $('.infoClickHover').on('click', function () {
        ...
        if (!isNaN(Number(stepData.responsible))) {
            var dataColleague = getDataColleague(stepData.responsible)
            var emailApprover = dataColleague.mail
            emailApprover = emailApprover.split('@');

            structureUser = `<img src="/social/api/rest/social/image/profile/${emailApprover[0]}/MEDIUM_PICTURE" class="photoModalStep"></img>
                            <div class="contentModalItem-text">
                                <a class="clickable" href="https://concessionariarota156340.fluig.cloudtotvs.com.br:1700/portal/p/1/social/${emailApprover[0]}" target="_blank">${dataColleague.colleagueName || 'Não definido'}</a>
                                ${stepData.typeResponsible === 'user' ? `<p class="modalStepPosition">CARGO USUÀRIO A CIMA</p>` : ''}
                            </div>`;
        } else {
            structureUser = `<img src="https://cdn.jsdelivr.net/gh/luccsll/styleguide-nro@4060759ad8eaf3dff0c460003f2df6ad93df14f3/img/group.png" class="photoModalStep">
                            <div class="contentModalItem-text">
            ...
            <div class="backgroundModal" id="modalStep${stepId}">
                <div class="contentModal">
                    <div class="contentModalHeader">
                        <h1 class="contentModalTitle">${stepData.activity} <button class="buttonCloseModalStep"><img src="https://cdn.jsdelivr.net/gh/luccsll/styleguide-nro@main/icons/flaticon-close.png"></img></button></h1>
...
function getDataColleague(colleague) {
    var filterColleague = new Array(DatasetFactory.createConstraint("currentProject", colleague, colleague, ConstraintType.MUST))
    var colleague = DatasetFactory.getDataset("colleague", null, filterColleague, null);
    return colleague.values[colleague.values.length - 1]
}